...
```
用户消息 → 飞书 Bot → Agent ReAct 循环 → Azure OpenAI（带工具定义）
                                ↕
                        工具执行（OKR 查询、消息发送、团队查询）
                                ↕
...
│   └── bot.go                  # Bot WebSocket 事件处理（单一 handler）
...
- [Azure OpenAI](https://azure.microsoft.com/en-us/products/ai-services/openai-service) 资源（需部署支持 tool calling 的模型，如 gpt-4o）
...
| `AZURE_OPENAI_ENDPOINT` | 是 | Azure OpenAI 端点 URL | — |
| `AZURE_OPENAI_API_KEY` | 是 | Azure OpenAI API Key | — |
| `AZURE_OPENAI_DEPLOYMENT` | 是 | 部署名称（如 gpt-4o） | — |
