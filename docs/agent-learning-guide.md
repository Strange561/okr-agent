...
**Azure OpenAI 的特殊性**（`llm/client.go`）：

```go
// URL 包含 deployment 名称和 API 版本
url := fmt.Sprintf("%s/openai/deployments/%s/chat/completions?api-version=%s",
    c.endpoint, c.deployment, c.apiVersion)

// 使用 api-key 头认证（非标准 OpenAI 的 Bearer Token）
...
如果要换成其他 LLM 提供商（OpenAI、Anthropic），只需修改 URL 格式和认证方式，消息协议是通用的。
...
注意：`seen` map 没有 TTL 清理——长时间运行后可能积累大量 key。生产环境应考虑加 LRU 或定期清理。
...
ReAct 循环 (最多 10 轮)
    ...
    ├─ 调用 Azure OpenAI (带 8 个工具定义 + ToolCall ID)
    │
...
`SetHandler` 是平台层与 Agent 层的桥梁——任何能提供 `(senderID, text) → response` 回调的消息平台都能接入。
